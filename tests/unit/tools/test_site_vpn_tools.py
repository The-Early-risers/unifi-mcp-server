...
class TestListSiteToSiteVpns:
    """Tests for list_site_to_site_vpns."""
...
class TestGetSiteToSiteVpn:
    """Tests for get_site_to_site_vpn."""
    ...
    async def test_returns_matching_vpn(self, mock_settings):
        ...
        with patch("src.tools.site_vpn.UniFiClient", return_value=mock_client):
            result = await get_site_to_site_vpn("default", "vpn-1", mock_settings)
...
class TestUpdateSiteToSiteVpn:
    """Tests for update_site_to_site_vpn."""
    ...
    async def test_raises_not_found_when_not_site_vpn(self, mock_settings):
        non_vpn = {**make_vpn("vpn-1"), "purpose": "corporate"}
        mock_client = create_mock_client(non_vpn)
        with patch("src.tools.site_vpn.UniFiClient", return_value=mock_client):
            with pytest.raises(ResourceNotFoundError):
                await update_site_to_site_vpn("default", "vpn-1", mock_settings, confirm=True)
